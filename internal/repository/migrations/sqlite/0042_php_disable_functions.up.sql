-- Per-site PHP disable_functions policy (SQLite).
--
...
--
-- Default 'strict': every site gets the baseline unless an operator (or, later, a
-- plan) deliberately relaxes it. A pool's on-disk file catches up to this on its
-- next render.
ALTER TABLE php_pools ADD COLUMN func_policy TEXT NOT NULL DEFAULT 'strict';
