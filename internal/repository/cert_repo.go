...
const certCols = `id, uid, owner_id, provider, common_name, sans, is_wildcard, cert_pem, privkey_enc,
	COALESCE(issued_at,'') AS issued_at, COALESCE(expires_at,'') AS expires_at, auto_renew, status, created_at`

func (s *CertStore) Insert(ctx context.Context, r *ssl.Record) error {
	if r.UID == "" {
		r.UID = idgen.NewULID()
	}
	// Upsert by common_name (re-issuing replaces the record).
	res, err := s.db.ExecContext(ctx,
		`UPDATE ssl_certificates SET provider=?, sans=?, cert_pem=?, privkey_enc=?, issued_at=?, expires_at=?, status=?
		  WHERE common_name = ?`,
		r.Provider, r.SANs, r.CertPEM, r.PrivkeyEnc, r.IssuedAt, r.ExpiresAt, r.Status, r.CommonName)
	if err != nil {
	...
	ins, err := s.db.ExecContext(ctx,
		`INSERT INTO ssl_certificates (uid, owner_id, provider, common_name, sans, cert_pem, privkey_enc, issued_at, expires_at, auto_renew, status)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		r.UID, r.OwnerID, r.Provider, r.CommonName, r.SANs, r.CertPEM, r.PrivkeyEnc, r.IssuedAt, r.ExpiresAt, r.AutoRenew, r.Status)
	if err != nil {
