...
func TestRcloneTarget(t *testing.T) {
	...
	tgt := NewRcloneTarget(RcloneConfig{Remote: "gdrive:nexpanel-backups", Config: "/etc/rclone.conf"})
	if tgt == nil {
	...
	if got := tgt.object("sites/S1/B1.enc"); got != "gdrive:nexpanel-backups/sites/S1/B1.enc" {
		t.Errorf("object = %q", got)
	}
	args := tgt.args("rcat", tgt.object("k"))
	joined := strings.Join(args, " ")
	for _, want := range []string{"--config /etc/rclone.conf", "rcat", "gdrive:nexpanel-backups/k"} {
