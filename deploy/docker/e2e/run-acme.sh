...
# server: account registration, order, an HTTP-01 challenge that NexPanel writes
...
# the challenge on :80), np-broker, and npd. npd trusts Pebble because Pebble's
# CA is installed into the system trust store before npd starts — so the ACME
...
# trust anchor we install — SAN covers the addresses npd dials Pebble on.
...
sec "start np-broker + npd (ACME pointed at Pebble via NP_SSL_DIRECTORY)"
...
NP_SERVER_HOST=127.0.0.1 NP_SERVER_PORT=18443 NP_LOG_FORMAT=text \
  ...
  NP_SSL_EMAIL="admin@$domain" NP_SSL_DIRECTORY="https://127.0.0.1:14000/dir" \
...
grep -o "Let's Encrypt enabled" /tmp/npd.log | head -1
...
chmod o+x /srv/nexpanel/sites/1 2>/dev/null
...
sec "RESULT"
grep -q "Let's Encrypt enabled" /tmp/npd.log || { echo "  FAIL ACME issuer not enabled"; fail=1; }
