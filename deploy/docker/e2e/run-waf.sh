#!/usr/bin/env bash
# Per-site WAF: NexPanel toggles ModSecurity + the OWASP Core Rule Set on a
...
NP_LOG_FORMAT=text NP_BROKER_ALLOWED_UID=0 np-broker --serve --socket /run/nexpanel/broker.sock >/tmp/broker-waf.log 2>&1 &
...
NP_SERVER_HOST=127.0.0.1 NP_SERVER_PORT=18466 NP_LOG_FORMAT=text \
  NP_DATABASE_DRIVER=sqlite NP_DATABASE_DSN=/tmp/np-waf.db \
  NP_BROKER_SOCKET=/run/nexpanel/broker.sock npd >/tmp/npd-waf.log 2>&1 &
...
CSRF=$(awk '/np_csrf/{print $7}' /tmp/cwaf.txt)
...
echo '<h1>hello behind the WAF</h1>' > /srv/nexpanel/sites/1/public/index.html
chmod o+x /srv/nexpanel/sites/1 && chmod o+rx /srv/nexpanel/sites/1/public
...
[ -f /etc/nexpanel/waf/main.conf ] && pass "the WAF rules file was written" || fail "no WAF rules file"
grep -q 'modsecurity-crs/rules' /etc/nexpanel/waf/main.conf && pass "the rules file pulls in the OWASP CRS" || fail "CRS not referenced"
grep -q 'module mod_security' /usr/local/lsws/conf/nexpanel.conf && pass "the vhost has the ModSecurity block" || fail "no modsec block in the vhost"
sleep 1
...
sleep 1
grep -q 'module mod_security' /usr/local/lsws/conf/nexpanel.conf && fail "the modsec block survived disabling" \
...
sec "cleanup"
pkill -f 'npd' 2>/dev/null; pkill -f 'np-broker' 2>/dev/null; /usr/local/lsws/bin/lswsctrl stop >/dev/null 2>&1; true
