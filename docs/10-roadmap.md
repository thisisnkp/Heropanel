...
- **Exit criteria:** provision a mail domain with passing DKIM/SPF/DMARC and send/receive — **met**: all four records resolve from live DNS with the key-pair correspondence proven, and a message is sent (SMTP), delivered (LMTP, DKIM-signed) and received (IMAP) end to end. *(Deferred to Phase 8: submission/IMAPS TLS via the SSL module, inbound verification policy.)*
