#!/usr/bin/env bash
# Real PHP: NexPanel provisions a PHP site whose vhost talks FastCGI to a
...
echo "--- /etc/php/8.3/fpm/pool.d/nps1.conf ---"; sed -n '1,12p' /etc/php/8.3/fpm/pool.d/nps1.conf 2>&1
echo "--- fpm sockets ---"; ls -la /run/nexpanel/fpm/ 2>&1
echo "--- generated OLS config (extProcessor + scriptHandler) ---"; grep -A5 -E "extProcessor|scriptHandler" /usr/local/lsws/conf/nexpanel.conf 2>&1 | head -18
...
printf '<?php echo "PHP ".PHP_VERSION." live via NexPanel + php-fpm + OpenLiteSpeed\\n"; ?>' \
