...
// runDecrypt implements `npd decrypt <sealed-file> <output-file>`: the
...
func runDecrypt(args []string) int {
	if len(args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: npd decrypt <sealed-file> <output-file>")
		fmt.Fprintln(os.Stderr, "  NP_SECRET_KEY must hold the panel master key the backup was made under.")
		return 2
	}
	in, out := args[0], args[1]

	master := os.Getenv("NP_SECRET_KEY")
	if master == "" {
		fmt.Fprintln(os.Stderr, "npd decrypt: NP_SECRET_KEY is not set — the sealed file cannot be opened without the panel master key.")
		return 1
	}
	key, err := secrets.DeriveKeyBase64(master, "backup-v1")
	if err != nil {
		fmt.Fprintln(os.Stderr, "npd decrypt: bad NP_SECRET_KEY:", err)
		return 1
	}

	src, err := os.Open(in)
	if err != nil {
		fmt.Fprintln(os.Stderr, "npd decrypt:", err)
		return 1
	}
	...
	if err := blobcrypt.Open(dst, src, key); err != nil {
		...
		if err == blobcrypt.ErrCorrupt {
			fmt.Fprintln(os.Stderr, "npd decrypt: the file failed authentication — wrong key, or the file is corrupt or was tampered with. Nothing was written.")
		} else {
			fmt.Fprintln(os.Stderr, "npd decrypt:", err)
		}
		return 1
