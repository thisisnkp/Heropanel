// Package policy holds the broker's capability policy: which privileged
// capabilities are enabled and the constraints that bound them (allowed service
// names, allowed filesystem roots, minimum uid). The policy is loaded from
// /etc/nexpanel/broker/policy.yaml (root-owned, 0600) at startup; this package
// defines the model and its checks. See docs/05-security-architecture.md.
package policy

import (
	"path"
	"strings"
)

// Policy is the broker's effective authorization policy.
type Policy struct {
	// Enabled maps capability name -> whether it may run at all. A capability
	// absent from this map is treated as disabled (deny by default).
	Enabled map[string]bool

	// Services is the allowlist of service names ServiceRestart may act on.
	Services []string

	// PathRoots is the allowlist of absolute roots under which file operations
	// (and site home directories) must reside.
	PathRoots []string

	// UIDMin is the minimum uid permitted for created system users, keeping
	// panel-created site users out of the system/reserved range.
	UIDMin int

	// PanelUser is the unprivileged account npd runs as. Capabilities that hand a
	// file back to the panel (a database export, say) chown it to this user, so
	// the panel never needs a root file handle.
	//
	// It is policy rather than a constant because the account name is a
	// deployment fact, not a property of the broker: a packager may install under
	// a different name, and a test harness runs npd as root.
	PanelUser string
}

// Default returns a conservative starting policy for a single-node install.
func Default() Policy {
	return Policy{
		Enabled: map[string]bool{
			"service.restart":  true,
			"service.status":   true,
			"system.provision": true,
			// Starts np-installer as a transient systemd unit to replace the
			// panel's own binaries. It writes nothing itself, so it needs no
			// PathRoots entry, and it restarts nothing itself, so npd and
			// np-broker stay off the Services allowlist — the installer the unit
			// runs is what does both, outside this process's lifetime.
			"panel.update":           true,
			"system_user.create":     true,
			"system_user.delete":     true,
			"site.create_dirs":       true,
			"site.remove_dirs":       true,
			"site.apply_slice":       true,
			"site.remove_slice":      true,
			"site.read_log":          true,
			"site.copy_tree":         true,
			"file.list":              true,
			"file.read":              true,
			"file.write":             true,
			"file.mkdir":             true,
			"file.remove":            true,
			"file.rename":            true,
			"file.copy":              true,
			"file.chmod":             true,
			"file.extract":           true,
			"file.compress":          true,
			"file.chown":             true,
			"file.search":            true,
			"terminal.open":          true,
			"webserver.apply":        true,
			"php.write_pool":         true,
			"php.list_extensions":    true,
			"php.set_extension":      true,
			"php.write_opcache":      true,
			"php.read_opcache":       true,
			"db.create":              true,
			"db.drop":                true,
			"db.user.create":         true,
			"db.user.drop":           true,
			"db.grant":               true,
			"db.revoke":              true,
			"db.size":                true,
			"db.export":              true,
			"db.import":              true,
			"cert.install":           true,
			"cert.write_challenge":   true,
			"git.deploy":             true,
			"git.rollback":           true,
			"app.unit_apply":         true,
			"app.unit_control":       true,
			"app.unit_remove":        true,
			"cron.apply":             true,
			"cron.remove":            true,
			"cron.run":               true,
			"cron.logs":              true,
			"backup.create":          true,
			"backup.restore":         true,
			"backup.prune":           true,
			"mail.provision":         true,
			"mail.apply":             true,
			"mail.tls":               true,
			"mail.inbound":           true,
			"mail.inbound.status":    true,
			"mail.authverify":        true,
			"mail.authverify.status": true,
			"mail.sso.apply":         true,
			"mail.purge":             true,
			"webmail.install":        true,
			"webmail.status":         true,
			"mail.dkim.apply":        true,
			"mail.queue.list":        true,
			"mail.queue.flush":       true,
			"mail.queue.delete":      true,
			"mail.quota":             true,
			"firewall.apply":         true,
			"firewall.confirm":       true,
			"firewall.rollback":      true,
			"firewall.status":        true,
			"malware.scan":           true,
			"malware.quarantine":     true,
			"malware.restore":        true,
			"malware.delete":         true,
			"maldet.status":          true,
			"maldet.scan":            true,
			"maldet.update":          true,
			"maldet.install":         true,
			"phpmyadmin.provision":   true,
			"fail2ban.status":        true,
			"fail2ban.ban":           true,
			"fail2ban.unban":         true,
			"ssh.harden":             true,
			"ssh.status":             true,
			"updates.configure":      true,
			"updates.status":         true,
			"waf.provision":          true,
			"fim.init":               true,
			"fim.check":              true,
			"fim.status":             true,
			"audit.scan":             true,
			"dns.write_zone":         true,
			"dns.remove_zone":        true,
			"dns.dnssec_status":      true,
			// Docker. Reachable only through these named operations: the daemon
			// socket is root-equivalent, so an operator who wants the panel to
			// keep its hands off containers entirely turns these off here and
			// the feature disappears rather than merely being hidden in the UI.
			"docker.info":                  true,
			"docker.container.list":        true,
			"docker.container.inspect":     true,
			"docker.container.logs":        true,
			"docker.container.stats":       true,
			"docker.container.start":       true,
			"docker.container.stop":        true,
			"docker.container.restart":     true,
			"docker.container.remove":      true,
			"docker.image.list":            true,
			"docker.image.pull":            true,
			"docker.image.remove":          true,
			"docker.image.prune":           true,
			"docker.container.create":      true,
			"docker.volume.list":           true,
			"docker.volume.create":         true,
			"docker.volume.remove":         true,
			"docker.volume.inspect":        true,
			"docker.network.list":          true,
			"docker.network.create":        true,
			"docker.network.remove":        true,
			"docker.network.inspect":       true,
			"docker.container.exec":        true,
			"docker.container.logs.follow": true,
			"docker.compose.up":            true,
			"docker.compose.down":          true,
			"docker.compose.ps":            true,
			"docker.compose.logs":          true,
		},
		Services: []string{
			// The opinionated stack: OpenLiteSpeed (or its licensed sibling),
			// MariaDB, and the mail/DNS/firewall services the wizard can enable.
			// Unit names span the Debian/RHEL split.
			"lshttpd", "openlitespeed", "lsws", "mariadb", "redis",
			"postfix", "dovecot", "nftables", "named",
			// The always-on security baseline: the intrusion blocker and ClamAV's
			// signature updater. clamscan itself is a command, not a unit.
			"fail2ban", "clamav-freshclam",
		},
		PathRoots: []string{"/srv/nexpanel/sites"},
		UIDMin:    20000,
		PanelUser: "nexpanel",
	}
}

// EffectivePanelUser returns the account npd runs as, falling back to the
// default when the policy leaves it unset.
func (p Policy) EffectivePanelUser() string {
	if p.PanelUser == "" {
		return "nexpanel"
	}
	return p.PanelUser
}

// CapabilityEnabled reports whether the named capability may run.
func (p Policy) CapabilityEnabled(name string) bool {
	return p.Enabled[name]
}

// ServiceAllowed reports whether name is on the service allowlist. It also
// accepts templated instances of an allowlisted unit (e.g. "php-fpm@site1" when
// "php-fpm" is allowed).
func (p Policy) ServiceAllowed(name string) bool {
	for _, s := range p.Services {
		if name == s {
			return true
		}
		if i := strings.IndexByte(name, '@'); i >= 0 && name[:i] == s {
			return true
		}
	}
	return false
}

// PathAllowed reports whether pth is confined to one of the allowlisted roots.
// The path is cleaned first, so traversal sequences (".." escaping a root) are
// rejected. Unix path semantics are used regardless of the build OS because the
// broker always operates on the Linux target's filesystem.
func (p Policy) PathAllowed(pth string) bool {
	cp := path.Clean(pth)
	if !path.IsAbs(cp) {
		return false
	}
	for _, root := range p.PathRoots {
		r := path.Clean(root)
		if cp == r || strings.HasPrefix(cp, r+"/") {
			return true
		}
	}
	return false
}
