...
export interface Database {
  ...
  /**
   * The website this database belongs to, by domain, or null when nothing owns
   * it.
   *
   * By domain rather than by uid because that is how the naming actually works:
   * a site's databases carry its domain-derived prefix (see dbPrefix), and the
   * link is visible in the name itself. It also survives the real endpoint
   * landing — `db_instances` has no site column yet, so any id here would be
   * invented.
   *
...
const SEED: Database[] = [
  { name: "nexp_novaretail", user: "nexp_admin", createdAt: "2026-03-14", siteDomain: "novaretail.in", size: "184 MB" },
  { name: "nexp_novaretail_stg", user: "nexp_admin", createdAt: "2026-06-02", siteDomain: "novaretail.in", size: "22 MB" },
  { name: "nexp_api", user: "nexp_api_rw", createdAt: "2026-04-21", siteDomain: "api.novaretail.in", size: "96 MB" },
  { name: "nexp_billing_portal", user: "nexp_admin", createdAt: "2026-02-27", siteDomain: "billing-portal.co", size: "41 MB" },
  ...
  { name: "legacy_wp_import", user: "nexp_admin", createdAt: "2025-11-08", siteDomain: null, size: "312 MB" },
  { name: "analytics_scratch", user: "nexp_reports", createdAt: "2026-01-19", siteDomain: null, size: "8 MB" },
];
...
export const useDatabasesStore = defineStore("databases", () => {
   ...
   */
  function forSite(domain: string) {
    return all.value.filter((d) => d.siteDomain === domain || d.siteDomain === null);
  }

  function isOrphan(db: Database) {
    return db.siteDomain === null;
  }

  function assign(name: string, siteDomain: string) {
    all.value = all.value.map((d) => (d.name === name ? { ...d, siteDomain } : d));
  }
  ...
  function add(name: string, user: string, siteDomain: string) {
    all.value = [
      ...all.value,
      { name, user, createdAt: new Date().toISOString().slice(0, 10), siteDomain, size: "0 B" },
    ];
  }
  ...
  const orphanCount = computed(() => all.value.filter((d) => d.siteDomain === null).length);
